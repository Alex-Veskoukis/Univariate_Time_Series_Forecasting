    ...
    if(length(input$Algorithm) > 0){
      if (reactiveVariables$timeSeries_submitted == T) {
        ...
        tryCatch({
          ...
          if("RW" %in% input$Algorithm) {
            message('Evaluating RW')
            forecasts$RW <- list()
            ...
            forecasts$RW[['Fit']] <- forecastFit$model
            forecasts$RW[['Forecast']] <- forecastFit_dt[, `Point Forecast`]
            forecasts$RW[['MAE']]  <- mae(reactiveVariables$Series_to_Evaluate, forecastFit_dt[, `Point Forecast`])
            forecasts$RW[['RMSE']] <- rmse(reactiveVariables$Series_to_Evaluate, forecastFit_dt[, `Point Forecast`])
            forecasts$RW[['MAPE']] <- mape(reactiveVariables$Series_to_Evaluate, forecastFit_dt[, `Point Forecast`])
          }
  ...
  observeEvent(input$forecast_ahead,{
    ...
    if(length(input$results_rows_selected) == 0){
      ...
      tryCatch({
        ...
        if("RW" %in% Algorithms) {
          message('Forecasting RW')
          forecasts_ahead$RW <- list()
          forecastFit <- rwf(reactiveVariables$TotalSeries,
                             h=input$forecast_horizon, 
                             model= reactiveVariables$Forecasts[['RW']][['Fit']])
          forecastFit_dt <- as.data.table(forecastFit)
          forecasts_ahead$RW[['Forecast']] <- forecastFit_dt[, .(`Point Forecast`, `Lo 95` , `Hi 95`)]
        }
